...
import { useDirectVodCategories, useDirectVodStreams, useDirectVodInfo, vodStreamToWatchContent } from '@/hooks/useIPTVDirect';
...
const IPTVMoviesContent = () => {
  ...
  const { data: categories, isLoading: catLoading } = useDirectVodCategories();
  const { data: streams, isLoading: streamsLoading } = useDirectVodStreams(selectedCategoryId);

  // Find category name for current selection
  ...
  // Transform streams to WatchContent
  const items = useMemo(() => {
    if (!streams || !iptvCredentials) return [];
    return streams.map(s => vodStreamToWatchContent(s, iptvCredentials, categoryName));
  }, [streams, iptvCredentials, categoryName]);
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 p-6 overflow-auto">
        <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
          ...
          {streamsLoading && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
        </h2>

        {!streamsLoading && items.length === 0 && (
          <p className="text-muted-foreground text-center py-12">No movies found in this category</p>
        )}
...
export default IPTVMoviesContent;
